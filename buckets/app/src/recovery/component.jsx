...
export default (auth) =>
  <Auth className={style.auth}>
    <Container className={style.auth__container} slot='main'>
      ...
      <form className={style.auth__form} onSubmit:formData={(data) => auth.recovery(data)}>
        <control.Email className={style.auth__control} />
        <button.Master className={style.auth__button}>Send login link</button.Master>
        <text.Strong className={style.auth__text} master darker small>OR</text.Strong>
        <link.Complete className={style.auth__link} href={urlFor('signup')}>Create New Account</link.Complete>
      </form>
